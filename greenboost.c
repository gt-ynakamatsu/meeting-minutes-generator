...
/*
 * GreenBoost v2.5 — 3-Tier GPU Memory Pool via DMA-BUF + CUDA UVM
 *
 ...
 *   GB_IOCTL_ALLOC   — pin System DDR pages (T2) or swappable 4K (T3), export DMA-BUF fd
...
#include <linux/dma-buf-map.h>   /* struct dma_buf_map (pre-iosys vmap) */
#endif
...
MODULE_DESCRIPTION("GreenBoost v2.5 — 3-tier pool: NVidia RTX GPU VRAM + System DDR RAM + NVMe swap");
...
/* ------------------------------------------------------------------ */
/*  Module parameters — 3-tier: RTX 5070 | System DDR | NVMe swap           */
/* ------------------------------------------------------------------ */
...
/* Tier 2 */
static int virtual_vram_gb   = 51;  /* 80% of 64 GB System DDR               */
...
static int gb_vmap_op(struct dma_buf *dmabuf, struct dma_buf_map *map)
{
	struct gb_buf *buf = dmabuf->priv;
	void *vaddr;
...
static void gb_vunmap_op(struct dma_buf *dmabuf, struct dma_buf_map *map)
{
	if (dma_buf_map_is_set(map) && !map->is_iomem)
		vunmap(map->vaddr);
...
static struct gb_buf *gb_alloc_buf(size_t size, u32 flags)
{
	...
	if (t2_used + size > t2_max) {
		/* Tier 2 (System DDR) full — try Tier 3 (NVMe-spillable) */
		...
		tier3 = true;
		gb_dbg("T2 System DDR full (%lluMB/%dGB) — spilling to T3 NVMe\n",
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	return sysfs_emit(buf,
		"=== GreenBoost v2.5 — 3-Tier Pool Info ===\n"
		"\n"
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	return sysfs_emit(buf,
		"=== GreenBoost v2.5 — Hardware Topology ===\n"
		"\n"
		"CPU  %s\n"
	...
	while (!kthread_should_stop()) {
		...
		if (swap_total_mb > 0) {
			...
			if (new_pressure != old_pressure) {
				...
				if (gb_dev.pressure_efd)
					eventfd_signal(gb_dev.pressure_efd, 1);
...
static int __init gb_init(void)
{
	...
	pr_info(DRIVER_NAME ": =====================================================\n");
	pr_info(DRIVER_NAME ": GreenBoost v2.5 — 3-Tier GPU Memory Pool (LRU+eventfd)\n");
	...
	gb_dev.cls = class_create(THIS_MODULE, CLASS_NAME);
...
{
	pr_info(DRIVER_NAME ": unloading GreenBoost v2.5\n");
